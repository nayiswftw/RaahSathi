...
import { MapPin, Star, DollarSign, Calendar, Users, Wallet, TrendingUp, Heart } from 'lucide-react';
...
const CardWrapper = memo(({ children }: { children: React.ReactNode }) => (
  <div
...
const ItinerarySection = memo(({ itineraryPlanner }: { itineraryPlanner?: any[] }) => (
  <>
...
const HotelsSection = memo(({ hotelOptions }: { hotelOptions?: any[] }) => (
  <>
    {hotelOptions?.map((hotel, i) => (
      <CardWrapper key={i}>
        <Link href={`https://www.google.com/maps/search/${hotel?.hotelName},${hotel?.hotelAddress}`} target="_blank">
...
const WeatherSection = memo(({ weatherForecast }: { weatherForecast?: any[] }) => (
  <>
...
const SpotsSection = memo(({ touristSpots }: { touristSpots?: any[] }) => (
  <>
    {touristSpots?.flatMap((day, i) =>
      day?.attractions?.map((attraction: any, idx: number) => (
        <CardWrapper key={`${i}-${idx}`}>
          <Link href={`https://www.google.com/maps/search/${attraction?.name}`} target="_blank">
...
const RestaurantsSection = memo(({ restaurantRecommendations }: { restaurantRecommendations?: any[] }) => (
  <>
    {restaurantRecommendations?.flatMap((day, i) =>
      day?.restaurants?.map((restaurant: any, idx: number) => (
        <CardWrapper key={`${i}-${idx}`}>
          <Link href={`https://www.google.com/maps/search/${restaurant?.name}`} target="_blank">
...
const BudgetSection = memo(({ budgetOverview }: { budgetOverview?: any }) => (
  <CardWrapper>
...
export const HeaderSection = memo(({ config, onSave }: { config: any, onSave?: () => void }) => {
